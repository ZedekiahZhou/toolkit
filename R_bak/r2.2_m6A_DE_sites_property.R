...
# motif info
motif = data.table::fread(paste0("03_Sites/merged_m6A.m6A.passed_len5_motif.bed")) %>%
  data.frame() %>%
...
for (case in c("PCIF1.KO", "STM2457")) {
  dfshort = read.delim(paste0(mydir, "DEtable_", case, "_vs_", ctrl, ".tsv")) %>%
    mutate(ID = paste(seqnames, end, sep = "."))
  dfshort = left_join(dfshort, motif, by = "ID") %>% 
    mutate(DRAC = ifelse(stringr::str_sub(motif, 1, 4) %in% drac, "DRAC", "Non-DRAC"))
  table(dfshort$DRAC)
  ...
  pdf(paste0("05_m6A/DEsites_", case, "_vs_", ctrl, ".pdf"), width = 9, height = 4)
